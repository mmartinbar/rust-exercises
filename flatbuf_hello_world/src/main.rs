...
fn main() {
    ...
    let flower_name = builder.create_string("Daisy");
    ...
    let daisy = Flower::create(&mut builder, &FlowerArgs {
        name: Some(flower_name),
        petals: 21
    });
    ...
    let orc = Monster::create(&mut builder, &MonsterArgs {
        ...
        equipped_type: Equipment::Flower,
        equipped: Some(daisy.as_union_value()),
        path: Some(path),
        ..Default::default()
    });
    ...
    let mut f = OpenOptions::new()
        ...
        .write(true)
        .truncate(true)
        .open("./monsterdata_test.mon").unwrap();
    ...
    let monster = get_root_as_monster(buf.as_ref());
    ...
    assert_eq!(monster.equipped_type(), Equipment::Flower);

    match monster.equipped_type() {
        Equipment::Weapon => println!("The monster is equipped with a weapon"),
        Equipment::Flower => println!("The monster is equipped with a flower"),
        Equipment::NONE => println!("The monster has no equipment")
    }
    let equipped = monster.equipped_as_flower().unwrap();
    let flower_name = equipped.name();
    let flower_petals = equipped.petals();
    assert_eq!(flower_name, Some("Daisy"));
    assert_eq!(flower_petals, 21);
